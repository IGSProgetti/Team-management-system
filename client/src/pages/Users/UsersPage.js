...
const CreateUserModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Notifica successo
      alert(`Utente ${formData.nome} creato con successo!`);
      ...
      setErrors({ submit: error.response?.data?.details || error.message || 'Errore durante la creazione' });
    } finally {
      setIsSubmitting(false);
    }
  };
