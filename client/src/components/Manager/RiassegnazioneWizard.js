...
const RiassegnazioneWizard = ({ isOpen, onClose, initialResource = null }) => {
  ...
  const fetchCrediti = async () => {
    try {
      const response = await api.get('/riassegnazioni/crediti-disponibili');
      setCrediti(response.data.crediti || []);
    } catch (error) {
  ...
  const fetchDebiti = async () => {
    try {
      const response = await api.get('/riassegnazioni/debiti');
      setDebiti(response.data.debiti || []);
    } catch (error) {
  ...
  const executeRiassegnazione = async () => {
    try {
      ...
      const response = await api.post('/riassegnazioni/create', payload);
